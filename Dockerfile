...
#    'airflow' 사용자/그룹 이름 대신 UID/GID(50000:0)를 직접 사용하여 권한 문제를 근본적으로 해결합니다.
...
#    COPY 명령어에 --chown 플래그를 사용하여 파일 소유권을 명확히 합니다.
...
#    airflow 사용자 권한으로 실행하여, 미리 만들어둔 캐시 폴더에 안전하게 저장합니다.
RUN python -c "from sentence_transformers import SentenceTransformer; SentenceTransformer('intfloat/multilingual-e-large')"