...
async def extract_tech_stack_with_llm_async(client, job_post_text: str, index: int) -> tuple[int, str | None]:
    """LLM을 사용하여 주어진 텍스트에서 기술 스택을 비동기적으로 추출합니다."""
...
async def preprocess_data_main(json_file_path: str):
    """
    JSON 파일을 읽어, 모든 데이터에 대한 LLM API 호출을 병렬로 실행하고,
    tqdm으로 진행률을 표시하며 결과를 취합하여 새로운 JSON 파일로 저장합니다.
    """
    ...
    print(f"총 {len(raw_data)}개의 데이터에 대한 비동기 API 호출을 준비합니다...")
    for i, job in enumerate(raw_data):
        ...
        tasks.append(extract_tech_stack_with_llm_async(llm_client, text_for_extraction, i))
